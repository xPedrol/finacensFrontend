...
export const generateExpensesGroup = (list: IExpense[]): IExpensesGroup[] => {
    ...
    list.forEach((item) => {
        ...
        if (date) {
            const formattedDate = dayjs(date).utc().format(DATE_OUTPUT_FORMAT);
            const isDateExist = expensesGroup.some((expenseGroup) => expenseGroup.date === formattedDate);
            if(!isDateExist) {
                const expenses = list.filter((expense) => dayjs(expense.date).utc().format(DATE_OUTPUT_FORMAT) === formattedDate);
